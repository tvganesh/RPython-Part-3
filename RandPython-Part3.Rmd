...
Usually there will be a large number of predictors for a target variable. It is likely that not all the predictor or feature variables will have an impact on the output. Hence it is becomes necessary to choose all those features which influence the output variable. The techiniques that are used to simplify the machine learning model while removing features which
have little or no impact are 
...
All these methods remove those features which do not have a great influence on the output.
# 1.1 Best Fit
For a dataset with features f1,f2,f3...fn, the best fit chooses all combinations of features and creates separate ML models for each of the combinations. The best fit algotithm then  uses some filtering  criteria based on Adj Rsquared, Cp or AIC to pick out the best model among all models.
Since the Best Fit approach searches the entire solumn space it is computationally infeasible. The number of models that have to be searched work to $latex 2^{p}$. This can be shown as follows
...
Since
$ (1+x)^{n} = 1+C_{1}x + C_{2} +... + C_{n}$ and when x=1 this becomes
$2^{n} = 1+C_{1}x + C_{2} +... + C_{n}$

Hence there are $2^{n}$ models to search amongs in Best Fit. For 10 features this is 1000 models and for 40 features this becoms $2^{40}$ which almost 1 trillion. Usually there are datasets with 1000 or maybe even 100000 features and Best fit becomes computationally infeasible.


Anyways I have included the Best Fit approach as I use the Boston crime datasets which is available both the MASS package in R and Sklearn in Python.

Perform a simple Linear Regression Fit. By looking at the p-values of the features it can be 
...
The Python package for performing a Best Fit is the Exhaustive Feature Selector.
...
so I decided to add this code to R. This is not available in the 
```{r fig3,cache=TRUE}
library(dplyr)
df=read.csv("Boston.csv",stringsAsFactors = FALSE) # Data from MASS - SL
names(df) <-c("no","crimeRate","zone","indus","charles","nox","rooms","age",
...
## 1.3 Backward Fit
Backward fit belongs to the class of greedy algorithms which  tries to optimize the feature set selected by dropping a feature at every stage which results in the worst performance for a given criteria of Adj RSquared, Cp, BIC or AIC.  For a dataset with features f1,f2,f3...fn, the backward fit starts with the all the features f1,f2.. fn to begin with. It then pick the ML model with a n-1 eatures  by dropping the feature the inclusion of which results in the worst performance in adj Rsquared, or minimum Cp, BIC or some such criteria.  At every step 1 feature is dopped. There is guarantee that the final list of features chosen will be the best among the lot. The computation required for this is of 
$n + n-1 + n -2 + .. 1 = n(n+1)/2$ which is of the order of $n^{2}$. Though backward fit is a sub optimal solution is far more computationally efficient than best fit
...
The Sequential Feature search also includes 'floating' variants which include or exclude features conditionally, once they were excluded or included. These variants are included below
```{python }
import numpy as np
import pandas as pd
import os
...
where $\lamda $ is teh regularization or tuning parameter. Increasing Lamda increases the penalty
on the coefficients thus shrinking them. However in Ridge Regression features that do not influence the target variable will shrink closer to zero but never become zero except for very large values of $\lambda$
